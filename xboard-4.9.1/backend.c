...
ChessSquare flangArray[2][BOARD_FILES] = {
    { EmptySquare, EmptySquare, WhitePawn, WhiteRook,
        WhiteKnight, WhiteFalcon, WhiteAmazon, WhiteKing },
    { BlackKing, BlackAmazon, BlackFalcon, BlackKnight,
        BlackRook, BlackPawn, EmptySquare, EmptySquare }
};
...
InitBackEnd1 ()
{
    ...
    */
	...
	if(strcmp(appData.variant, "normal") && !appData.noChessProgram) { // [HGM] hope this is an engine-defined variant
      ...
      case VariantFlang:      /* engine-defined: custom pieces, no check */
...
//      SetCharTableEsc(pieceToChar, "P.BRQSEXOGCATHD.VMLIFN.........^T..^L......^A^H/^F^G^M.^E^X^O^I.^P.^B^R..^D^S^C^VK"
  //                                 "p.brqsexogcathd.vmlifn.........^t..^l......^a^h/^f^g^m.^e^x^o^i.^p.^b^r..^d^s^c^vk", SUFFIXES);
    ...
    case VariantFlang:
      pieces = flangArray;
      nrCastlingRights = 0;
      SetCharTable(pieceToChar, "PN.R.............F...........AKpn.r.............f...........ak");
      break;
    case VariantGreat:
      pieces = GreatArray;
      gameInfo.boardWidth = 10;
    ...
    for( j=0; j<BOARD_WIDTH; j++ ) { ChessSquare s = EmptySquare;
        ...
        if(gameInfo.variant == VariantFlang) {
            if(j < BOARD_LEFT+2)
                initialPosition[pawnRow][j] = EmptySquare;
            if(j >= BOARD_RGHT-2)
                initialPosition[BOARD_HEIGHT-pawnRow-1][j] = EmptySquare;
        }
...
FakeBookMove: // [HGM] book: we jump here to simulate machine moves after book hit
    ...
    if(sscanf(message, "piece %s %s", buf2, buf1) == 2) {
      ...
      /* enforce legality on our own! */       && gameInfo.variant != VariantUnknown
                                               ...
                                               && gameInfo.variant != VariantFairy
                                               && gameInfo.variant != VariantFlang    ) return;
